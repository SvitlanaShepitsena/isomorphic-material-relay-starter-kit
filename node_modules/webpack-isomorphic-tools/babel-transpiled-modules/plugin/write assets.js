...
function populate_assets(output, json, options, log) {
	...
	var require_hook = _requireHacker2['default'].global_hook('webpack-module', function (path, module) {
		log.debug('require()ing "' + path + '"');

		// if Webpack aliases are supplied
		if (options.alias) {
			// possibly alias the path
			var aliased_global_path = _common.alias_hook(path, module, options.project_path, options.alias, log);
		...
		//
		if (_helpers.exists(global_paths_to_parsed_asset_paths[path])) {
		...
		if (candidates.length === 0) {} else if (candidates.length > 1) {
